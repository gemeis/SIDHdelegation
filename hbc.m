...
function ISODET_HBC_Pre(e2,C2,C3)
    ...
    kf := Integers()!Random(Integers(2^e2));
    kb := Integers()!Random(Integers(2^e2));
    return kf,kb,Points;
end function;
...
function time_ISODET_HBC(a0,e2,e3,f)
    ...
    a := Integers()!Random(Integers(2^e2));
                ...
                t := ClockCycles();
    kf,kb,POINTS := ISODET_HBC_Pre(e2,C2,C3);
                T := ClockCycles()-t;
    CK,POINTS := ISODET_HBC_U1(kf,e2,e3,f,C2,strat2,POINTS);
                t := ClockCycles();
    _ := ISODET_HBC_12(a,CK);
                T +:= ClockCycles()-t;
    CKA := ISODET_HBC_U2(a,e2,CK,strat2,POINTS);
                t := ClockCycles();
    K := ISODET_HBC_23(kb,CKA);
                T +:= ClockCycles()-t;
    CA := ISODET_HBC_U3(kb,e2,CKA,strat2);
