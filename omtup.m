...
function time_ISODET_OMTUP(a0,e2,e3,f)
    ...
    // Isogeny #1
    tt:= ClockCycles();
     k := Integers()!Random(Integers(2^e2)); // detour kernel
     c,d,r,s,r0,s0 := ISO_OMTUP_Pre(a,e2,e2 div 8,4); // shrouding a, t=e2/8, N=4
    T := ClockCycles()-tt;

    N := 4;
    a24,cE1,dE1,r0Q,rQ,Points := ISO_OMTUP_U(k,e2,N,r0,r,C2,strat2,[xPI,xQI]);
     _ ,cE2,dE2,s0Q,sQ,Points := ISO_OMTUP_U(k,e2,N,s0,s,C2,strat2,[xPI,xQI]);
    CK := [a24] cat Points;
    ...
    // Isogeny #2
    tt:= ClockCycles();
     c,d,r,s,r0,s0 := ISO_OMTUP_Pre(k,e2,e2 div 8,4); // shrouding kb, t=e2/8, N=4
    T+:= ClockCycles()-tt;
    ...
    a24,cE1,dE1,r0Q,rQ,Points := ISO_OMTUP_U(k,e2,N,r0,r,CAK,strat2,[]);
     _ ,cE2,dE2,s0Q,sQ,Points := ISO_OMTUP_U(k,e2,N,s0,s,CAK,strat2,[]);
...
function time_PUBKEY_OMTUP(a0,e2,e3,f)
    ...
    N := 4;
    a24,cE1,dE1,r0Q,rQ,Points := ISO_OMTUP_U(k,e2,N,r0,r,C2,strat2,[xPI,xQI,xPB,xQB]);
     _ ,cE2,dE2,s0Q,sQ,Points := ISO_OMTUP_U(k,e2,N,s0,s,C2,strat2,[xPI,xQI,xPB,xQB]);
    CK := [a24] cat Points;
    ...
    // Isogeny #3
    a24,cE1,dE1,r0Q,rQ,Points := ISO_OMTUP_U(k,e2,N,r0,r,CAK,strat2,[xPB,xQB]);
     _ ,cE2,dE2,s0Q,sQ,Points := ISO_OMTUP_U(k,e2,N,s0,s,CAK,strat2,[xPB,xQB]);
